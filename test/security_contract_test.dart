...
void main() {
  ...
  test('revoked device state is terminal', () {
    expect(
      deviceTransitionAllowed(DeviceLifecycleState.pending, DeviceLifecycleState.active),
      isTrue,
    );
    expect(
      deviceTransitionAllowed(DeviceLifecycleState.active, DeviceLifecycleState.suspended),
      isTrue,
    );
    expect(
      deviceTransitionAllowed(DeviceLifecycleState.revoked, DeviceLifecycleState.active),
      isFalse,
    );
  });
  ...
  test('encrypted upload plan uses contiguous randomized R2 keys', () {
    ...
    final plan = planEncryptedUpload(
      ...
      randomSegment: () => 'randomized-storage-segment-${counter++}'.padRight(32, 'x'),
    );
    expect(plan.chunks.length, 3);
    expect(plan.chunks.map((chunk) => chunk.index), orderedEquals([0, 1, 2]));
    expect(plan.chunks.map((chunk) => chunk.randomizedStorageKey).toSet().length, 3);
    expect(plan.chunks.every((chunk) => !chunk.randomizedStorageKey.contains('object-1')), isTrue);
  });
}
