...
final class _SecurityCenterPageState extends State<SecurityCenterPage> {
  late Future<({List<DeviceSummary> devices, List<RecoveryChannelSummary> channels})> _load;
          ...
          return ListView(
            ...
            children: [
              ...
              for (final channel in data.channels)
                ListTile(
                  leading: Icon(
                    channel.kind == RecoveryChannelKind.email ? Icons.email_outlined : Icons.sms_outlined,
                  ),
                  title: Text(channel.maskedDestination),
                  subtitle: Text(channel.isVerified ? 'Verified' : 'Verification required'),
                ),
              Wrap(
                spacing: 8,
                children: [
                  OutlinedButton.icon(
                    onPressed: () => widget.onAddRecoveryChannel(RecoveryChannelKind.email),
                    icon: const Icon(Icons.email_outlined),
                    label: const Text('Add backup email'),
                  ),
                  OutlinedButton.icon(
                    onPressed: () => widget.onAddRecoveryChannel(RecoveryChannelKind.phone),
